...
# Skill Creator (CLI fork)

> **Fork notice.** This is a fork of Anthropic's skill-creator (Apache-2.0,
> see `LICENSE.txt`), renamed to `skill-creator-cli` to avoid collision
> with the upstream skill. Modifications: dropped the `anthropic` SDK
...
#### Critical: queries must be substantive enough to need a skill

Claude only invokes a skill for tasks it can't easily handle on its own. If a should-trigger query is short or generic enough that Claude can answer with a quick Read/Grep/Bash, **the skill won't fire no matter how good the description is**, and the optimization loop will report `recall = 0%` across every iteration. We've hit this — 4 generations of progressively more pushy descriptions, all with the same 0% recall, because the queries just weren't heavy enough to make Claude consult any skill.
...
**Subscription-only authentication**: This fork of skill-creator drops the
`anthropic` SDK dependency. Description improvement calls go through
`claude -p --effort max --output-format json` (subscription auth, no
`ANTHROPIC_API_KEY` required). Effort defaults to `max`; pass
`--effort high` (etc.) to `improve_description.py` to lower it.

This handles the full optimization loop automatically. It splits the eval set into 60% train and 40% held-out test, evaluates the current description (running each query 3 times to get a reliable trigger rate), then asks Claude (via `claude -p --effort max`) to propose improvements based on what failed. It re-evaluates each new description on both train and test, iterating up to 5 times. When it's done, it opens an HTML report in the browser showing the results per iteration and returns JSON with `best_description` — selected by test score rather than train score to avoid overfitting.
