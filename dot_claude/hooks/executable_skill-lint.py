...
# references には性質の違う 2 種類が同居している。プロンプトの骨組みは埋めて別の
# エージェントへ渡るので長さが相手のコンテキストになるが、引ける契約は main が必要な
# ときに 1 回読むだけで、長さは文書化している対象の大きさで決まる。
# 名前で分けると命名規約に依存する分岐になるので、緩いほうへ揃える。
# 骨組みが 200 行まで育つのは異常事態で、そのときは別の手で気づく。
