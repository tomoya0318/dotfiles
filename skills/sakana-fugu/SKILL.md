...
Sakana Fugu（`fugu` / `fugu-ultra`）に OpenAI 互換 Responses API でプロンプトを送り、回答を受け取るスキル。汎用の問い合わせ（調査・要約・設計相談・実装委譲など）に使う。

## 前提

- API キー: 1Password の `Private` vault にある `Sakana Fugu API` アイテムの password フィールド。`op` CLI でサインイン済みであること。
- Secret Reference: `op://Private/Sakana Fugu API/password`。API キー自体はコード・プロンプト・ログ・環境変数に保存しない。
- `uv` がインストール済み。依存（openai）は PEP723 で隔離環境に自動解決される（初回のみ取得・キャッシュ）。
- スクリプト: `~/.agents/skills/sakana-fugu/fugu.py`（Claude側にも同じスキルをリンク）
...
# 2. パイプで渡す
uv run ~/.agents/skills/sakana-fugu/fugu.py --model fugu < /path/to/prompt.txt
```

短い1行プロンプトなら位置引数でも可:

```bash
uv run ~/.agents/skills/sakana-fugu/fugu.py --model fugu-ultra "プロンプト本文"
```
...
- API キーを取得できない場合は終了コード 2 でエラーになる。`op` のサインイン状態と Secret Reference を確認する。
